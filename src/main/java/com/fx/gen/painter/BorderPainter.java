...
public class BorderPainter implements Painter<JXMapViewer> {
    ...
    public BorderPainter(List<Position> border, Color color) {
        this.border = border;
        this.color = color;
        calcCenter();
    }
    ...
    private void calcCenter() {
        double lonMin = border.stream().min(Comparator.comparing(Position::getLon)).get().getLon();
        double lonMax = border.stream().max(Comparator.comparing(Position::getLon)).get().getLon();

        double latMin = border.stream().min(Comparator.comparing(Position::getLat)).get().getLat();
        double latMax = border.stream().max(Comparator.comparing(Position::getLat)).get().getLat();
